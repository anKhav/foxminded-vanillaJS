...
const Cart = (filterDOM, cart) => {
    const additionalMenu = document.createElement('div')
    additionalMenu.classList.add('nav-additional')
    additionalMenu.innerHTML = `<input id="cart" type="checkbox" >
      <div class="cart">
        <button id="cart-button--close" class="cart__button--close"></button>
        <span class="cart__subtitle">box</span>
        <h3 class="cart__title">Your bag</h3>
        <div class="cart__content">
      ${CartProductList(cart)}
</div>
        <p class="cart__total-price">Total:<span>$190.80</span></p>
    ...
    const cartCheckbox = additionalMenu.querySelector('#cart')
    additionalMenu.addEventListener('click', (e) => {
        const mainBox = document.querySelector('main')
        if (e.target.id === 'cart'){
            filterDOM.classList.add('filter--on')
            document.body.classList.add('no-scroll')
            mainBox.style.zIndex = '-1'
        } else if (e.target.classList.contains('filter--on') || e.target.id === 'cart-button--close'){
